...
**Status:** Living document â€” keep in sync with `packages/backend/src/saas/` and `packages/frontend/app/saas/`.
...
1. **Self-service signup** â€” `POST /auth/register` from the public marketing page (`/`). The signup form on `/signup` is the canonical path. It auto-assigns a 14-day Pro trial per plan rev 3.
2. **Manual provisioning** â€” used by support or sales.
...
The endpoint `POST /saas/tenants` returns the new `tenant.id`. Seed the owner user with the same endpoint â€” that flow is `SaasService.createTenant`.
...
- Click the **Suspend** icon on a row â†’ calls `POST /saas/tenants/:id/suspend`. Backend sets `subscriptionStatus='cancelled'`. The salon is read-only until reactivated.
- Click **Reactivate** â†’ `POST /saas/tenants/:id/reactivate`. Sets `subscriptionStatus='active'`.
...
- **Total tenants / users / clients / appointments** â€” counts across every non-deleted tenant.
- **MRR** â€” `Î£ (active tenants on plan P) Ã— PLAN_PRICES[P]`, where `PLAN_PRICES` lives in `packages/backend/src/saas/saas.constants.ts`. **Trialing tenants contribute 0**; past-due and cancelled are excluded from MRR but counted elsewhere.
- **Active subscriptions / trial / churned** â€” counts by `subscriptionStatus`.
- **Top countries / plan distribution** â€” groupBy aggregations.
- **New tenants / users this month** â€” count where `createdAt >= startOfMonth`.
...
6. **Exit** removes both tokens, logs the user out, and routes to `/saas/login`. The SaaS owner must log in again (we cannot restore their session â€” the impersonation token was the only auth they had).
...
4. **All child rows are preserved** â€” users, appointments, payments, etc. â€” so an accidental delete is reversible.
...
- `GET /saas/tenants?includeDeleted=true` â€” wired through `TenantFilterDto.includeDeleted`. The frontend `/saas/tenants` page can be extended with a "show deleted" toggle. (Plan: defer UI until needed.)
...
| ---------- | ------------ | -------------------------------------- |
| `esencial` | â‚¬29 / mo     | Basic booking, 1 location              |
| `pro`      | â‚¬59 / mo     | Adds WhatsApp reminders, AI receptionist |
| `empresa`  | â‚¬119 / mo    | Multi-location, requires â‰¥ 2           |
...
- **Middleware**: `packages/frontend/middleware.ts` is intentionally permissive for SaaS routes â€” authentication is enforced by the backend `SaasOwnerGuard`. The dashboard layout's `useEffect` is a UX redirect, not a security gate.
...
- **`AuditLog.actorId` is a hard FK to `User` with `onDelete: Restrict`** â€” deleting a SaaS owner user will fail if they have audit entries. This is intentional: the audit trail must never dangle.
...
The CI pipeline in `.github/workflows/ci-cd.yml` runs migrations only on the test pipeline â€” production deploys must run them explicitly as a gate. If your platform supports pre-deploy hooks (Kubernetes init containers, ECS `command` overrides, `docker-compose` `entrypoint` wrappers), wrap the start command:
